...
describe.skipIf(!testDatabaseUrl)("database recovery paths", () => {
  ...
  it("applies 001 through 006 on an empty database", async () => {
    ...
    try {
      ...
      expect(result.applied).toEqual(["001_initial_persistence.sql", "002_api_idempotency.sql", "003_indexer_state.sql", "004_marketplace_commerce.sql", "005_wallet_auth.sql", "006_private_media.sql"]);
  ...
  it("baselines a compatible pre-existing 001 schema and then migrates 002 through 006", async () => {
    ...
    try {
      ...
      await expect(migrate({ pool, config })).rejects.toThrow(/already exists/);
      ...
      const validation = await validateDatabase({ pool, config });
      expect(validation.ok).toBe(true);
      expect(validation.migrations.map((item) => item.name)).toHaveLength(6);
    } finally { await pool.end(); }
  }, 120000);
  ...
  it("refuses to baseline a partially created schema and writes no migration record", async () => {
    ...
    try {
      ...
      await pool.query(`${sql.slice(0, sql.indexOf("CREATE TABLE releases"))}\nCOMMIT;`);
