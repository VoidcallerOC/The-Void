import { retry } from "./indexer.js";

export async function reconcileOwnership({ rpc, store, chainId, contractAddress, assets, wallets, blockTag = "finalized", retryOptions = {} } = {}) {
  const results = [];
  for (const wallet of wallets) {
    for (const tokenId of assets) {
      ...
      if (!match) await store.recordIndexerError({ chainId, contractAddress, errorType: "OWNERSHIP_MISMATCH", message: "Indexed ownership differs from blockchain balance.", payload: results.at(-1) });
    }
  }
  return results;
}
