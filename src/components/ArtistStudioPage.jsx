...
import { FUJI_RELEASE_CONFIG, assertFujiAddress, encodeCreateFujiEdition, fujiTokenId, sendFujiTransaction } from "../lib/fuji-release.js";
...
function initialForms() {
  return {
    ...
    edition: { id: "", name: "", description: "", artwork: "", chainId: String(FUJI_RELEASE_CONFIG.chainId), contractAddress: FUJI_RELEASE_CONFIG.contractAddress, tokenId: "", quantity: "", metadataUri: "", priceWei: "", marketplace: "{}", status: "DRAFT" },
...
export function ArtistStudioPage() {
  ...
  const publishFujiEdition = async () => {
    setBusy("fuji-publish"); setNotice("");
    try {
      if (!releaseId || !editionId) throw new Error("Save the release and edition before publishing on Fuji.");
      if (Number(forms.edition.chainId) !== FUJI_RELEASE_CONFIG.chainId) throw new Error(`Fuji publishing requires chain ${FUJI_RELEASE_CONFIG.chainId}.`);
      assertFujiAddress(forms.edition.contractAddress);
      const { tokenId, data } = encodeCreateFujiEdition({ releaseId, editionId, maxSupply: forms.edition.quantity, metadataUri: forms.edition.metadataUri || `ipfs://the-void-${editionId}` });
      const result = await sendFujiTransaction({ provider: wallet.getProvider(), from: wallet.account, data });
      const record = await studioFetch(`/studio/editions/${encodeURIComponent(editionId)}`, { method: "PATCH", payload: { status: "PUBLISHED", metadata: { fuji: { contractAddress: FUJI_RELEASE_CONFIG.contractAddress, chainId: FUJI_RELEASE_CONFIG.chainId, tokenId: tokenId.toString(), transactionHash: result.hash, metadataUri: forms.edition.metadataUri || `ipfs://the-void-${editionId}` } } }, headers });
      report(`Fuji edition published after receipt: ${record.title || editionId}. Token ID ${tokenId.toString()}.`);
    } catch (error) { report(error.message); } finally { setBusy(""); }
  };
  ...
  return <section style={shell}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, opacity: canUseStudio ? 1 : .5, pointerEvents: canUseStudio ? "auto" : "none" }}>
      ...
      <StudioCard eyebrow="03 · EDITION" title="Relic"><form onSubmit={submitEdition}><TextField title="Edition name" value={forms.edition.name} onChange={(value) => update("edition", "name", value)} required /><TextField title="Description" value={forms.edition.description} onChange={(value) => update("edition", "description", value)} multiline /><TextField title="Artwork URL" value={forms.edition.artwork} onChange={(value) => update("edition", "artwork", value)} /><TextField title="Certified chain ID" value={forms.edition.chainId} onChange={() => {}} required /><TextField title="Certified ERC-1155 contract" value={forms.edition.contractAddress} onChange={() => {}} required /><TextField title="Deterministic token ID" value={releaseId && editionId ? fujiTokenId(releaseId, editionId).toString() : "Calculated after save"} onChange={() => {}} required /><TextField title="Quantity" value={forms.edition.quantity} onChange={(value) => update("edition", "quantity", value)} required /><TextField title="Metadata URI" value={forms.edition.metadataUri} onChange={(value) => update("edition", "metadataUri", value)} required placeholder="ipfs://…" /><TextField title="Price (wei)" value={forms.edition.priceWei} onChange={(value) => update("edition", "priceWei", value)} required /><TextField title="Marketplace configuration (JSON)" value={forms.edition.marketplace} onChange={(value) => update("edition", "marketplace", value)} multiline /><Lifecycle value={forms.edition.status} onChange={(value) => update("edition", "status", value)} /><button disabled={busy === "edition"} style={{ ...button, marginTop: 18 }}>{busy === "edition" ? "Saving…" : editionId ? "Save edition" : "Create edition"}</button>{editionId && <button type="button" disabled={busy !== ""} onClick={publishFujiEdition} style={{ ...button, marginTop: 10, background: "transparent" }}>{busy === "fuji-publish" ? "ON-CHAIN CREATION · CONFIRMING…" : "ON-CHAIN CREATION · CERTIFIED FUJI"}</button>}</form></StudioCard>
