...
export function FujiIntegrationPage() {
  ...
  const createEdition = async () => {
    ...
    try {
      if (!wallet.account) throw new Error("Connect and authenticate the authorized artist wallet first.");
  ...
  const mint = async () => {
    ...
    try {
      if (!wallet.account) throw new Error("Connect and authenticate the authorized issuer wallet first.");
  ...
  const grant = async () => {
    ...
    try {
      ...
      setNotice(`Protected media grant confirmed: ${result.grantId}.`);
    } catch (error) { setNotice(error.message); } finally { setBusy(""); }
  };

  return <section style={shell}><header style={{ maxWidth: 760, marginBottom: 42 }}><p style={{ fontFamily: "var(--font-mono)", color: "var(--vc-crimson)", fontSize: 11, letterSpacing: ".18em" }}>† CERTIFIED FUJI INTEGRATION · STAGING ONLY</p><h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(50px, 9vw, 90px)", textTransform: "uppercase", lineHeight: .9, margin: "18px 0" }}>{release.title}</h1><p style={{ color: "var(--vc-bone-dim)", lineHeight: 1.7 }}>This page is the first real Fuji-backed vertical slice. It never substitutes the production Voidcaller catalog and never reports success before an on-chain receipt.</p></header><div style={{ ...card, marginBottom: 16 }}><strong>Certified contract</strong><p style={{ color: "var(--vc-bone-dim)", wordBreak: "break-all" }}>{FUJI_RELEASE_CONFIG.contractAddress} · chain {FUJI_RELEASE_CONFIG.chainId}</p><p style={{ color: "var(--vc-bone-dim)", wordBreak: "break-all" }}>Token ID: {tokenId.toString()}</p><p style={{ color: "var(--vc-bone-dim)" }}>Status: {paused === null ? "Not read" : paused ? "PAUSED" : "ACTIVE"} · Ownership: {balance === null ? "Not read" : owned ? `OWNED (${balance.toString()})` : "LOCKED"}</p></div>{!wallet.connected && <div style={{ ...card, borderColor: "var(--vc-crimson)", marginBottom: 16 }}>Connect an authorized Fuji wallet to exercise the real transaction path.</div>}<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}><section style={card}><h2 style={{ fontFamily: "var(--font-display)", textTransform: "uppercase" }}>Edition lifecycle</h2><p style={{ color: "var(--vc-bone-dim)" }}>Artist role → createEdition → receipt → deterministic token ID.</p><button style={button} disabled={busy !== "" || !wallet.connected} onClick={createEdition}>{busy === "create" ? "CONFIRMING…" : "CREATE EDITION ON FUJI"}</button><p><button style={{ ...button, marginTop: 12, background: "transparent" }} disabled={busy !== "" || !wallet.connected} onClick={mint}>{busy === "mint" ? "CONFIRMING…" : "MINT TO CONNECTED WALLET"}</button></p></section><section style={card}><h2 style={{ fontFamily: "var(--font-display)", textTransform: "uppercase" }}>Ownership and experience</h2><p style={{ color: "var(--vc-bone-dim)" }}>Direct balanceOf → owner unlock; non-owner remains locked.</p><button style={button} disabled={busy !== "" || !wallet.connected} onClick={refresh}>READ FUJI OWNERSHIP</button><p><button style={{ ...button, marginTop: 12, background: "transparent" }} disabled={busy !== "" || !owned} onClick={grant}>{busy === "grant" ? "AUTHORIZING…" : "REQUEST PROTECTED MEDIA"}</button></p></section></div>{roles && <p style={{ ...card, marginTop: 16, color: "var(--vc-bone-dim)" }}>Connected wallet roles: artist={String(roles.artist)} · issuer={String(roles.issuer)}</p>}{notice && <div role="status" style={{ ...card, marginTop: 16, borderColor: notice.includes("confirmed") ? "var(--vc-bone-dim)" : "var(--vc-crimson)" }}>{notice}</div>}<div style={{ ...card, marginTop: 16, color: "var(--vc-bone-dim)", fontFamily: "var(--font-mono)", fontSize: 11 }}>{tx.createEdition && <div>CREATE EDITION: <a href={fujiExplorerUrl("tx", tx.createEdition)} target="_blank" rel="noreferrer">{tx.createEdition}</a></div>}{tx.mint && <div>MINT: <a href={fujiExplorerUrl("tx", tx.mint)} target="_blank" rel="noreferrer">{tx.mint}</a></div>}</div></section>;
}
