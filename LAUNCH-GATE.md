...
| SECURE MEDIA | 🟡 CODE COMPLETE / INFRASTRUCTURE REQUIRED | Public preview files remain separate from protected masters. Wallet-bound, revocable, five-minute grants gate private media and are audited. Production uses Pinata private-media links and fails closed on provider or authorization errors. | Configure a valid Pinata credential and gateway, keep masters private, and test anonymous, owner, expired, revoked, provider-failure, and range requests in the real environment. |
| ARTIST STUDIO | 🟡 CODE COMPLETE / INFRASTRUCTURE REQUIRED | `/studio` implements Artist → Release → Track → Experience, artist-wallet authorization, `DRAFT → REVIEW → PUBLISHED`, ERC-1155 compatibility mapping, token validation, and audit events. | Apply migration `012_artist_studio.sql`; seed or create actual artist owner records through authenticated Studio use; test the deployed API with separate owner and non-owner wallets. |
...
| AVALANCHE C-CHAIN PRODUCTION | 🔴 NOT COMPLETE | The repository’s current production runtime remains Fuji-constrained; no C-Chain production deployment or certification evidence is supplied. | Complete a separate contract/deployment review, endpoint configuration, operational readiness checks, and an explicit staging-promotion change before claiming C-Chain production. |
...
The server routes are `POST /api/studio/artists`, `PATCH /api/studio/artists/:artistId`, `POST /api/studio/artists/:artistId/releases`, `PATCH /api/studio/releases/:releaseId`, `POST /api/studio/releases/:releaseId/editions`, `PATCH /api/studio/editions/:editionId`, `POST /api/studio/editions/:editionId/experiences`, and `PATCH /api/studio/experiences/:experienceId`. The route names retain internal ERC-1155 compatibility terminology; the Studio product surface presents Release, Track, and Experience. Every write requires the same verified bearer session used by the marketplace and media services. Migration `012_artist_studio.sql` adds the Studio ownership and lifecycle persistence.
...
| `AUTH_ALLOWED_CHAIN_IDS` | Wallet authentication chain allowlist | Current deployed Fuji environment uses `43113`; any C-Chain promotion requires a separate reviewed change. |
...
| `INDEXER_CHAIN_ID` | Indexer chain | Current Fuji deployment uses `43113`; do not claim C-Chain production without a separate reviewed deployment. |
...
| `MEDIA_STORAGE_DRIVER` | Protected media driver | Must be `pinata` in production. |
...
| `METADATA_STORAGE_DRIVER` | Canonical metadata provider | Set to `pinata` for production metadata publication. |
| `PINATA_JWT` | Server-only Pinata credential | Required secret for production metadata and protected media; never client-exposed. |
| `PINATA_GATEWAY_URL` | Pinata private-media gateway | Required HTTPS gateway in production. |
| `MEDIA_OBJECT_URL_HOSTS` | Approved Pinata gateway host | Must include the configured Pinata gateway host. |

## External infrastructure dependencies

The deployed application needs an internet-reachable Vercel frontend, a separately hosted long-running Node API, a private PostgreSQL database, a persistent Fuji indexer-worker runtime, the configured Avalanche Fuji RPC provider, reviewed and deployed ERC-1155 and marketplace contracts where applicable, Pinata credentials for JSON metadata and protected media, secret management, centralized logs, metrics and alerting, encrypted database backups, and a rehearsed restore process. The static Vercel configuration in this repository does not by itself run `server/index.js` or `server/indexer-worker.js`; those processes require their own runtime and release configuration.

## Exact remaining actions for a real launch

1. **Create a non-production Fuji environment.** Provision isolated PostgreSQL, Pinata configuration, API runtime, and persistent worker runtime. Set all required non-secret and secret configuration through the host’s secret manager.
...
4. **Configure protected media.** Store full-duration masters through the Pinata private-media path with public access disabled. Create published experience records whose protected media references resolve to those objects. Confirm server-side authentication, ownership, entitlement, short-lived Pinata download links, audit records, and fail-closed unauthorized behavior.
...
6. **Operationalize recovery.** Configure metrics and alerts for readiness, DB connections, RPC errors, indexer lag, lease loss, reorg rebuild failure, media authorization failures, Pinata/provider errors, and backup failures. Write and perform a database restore drill; record the recovery objective and observed restore time.
7. **Promote only after separate production review.** The current production configuration remains Fuji-constrained by the repository’s runtime validation. Any future C-Chain promotion requires an explicit implementation and deployment review, separate contract/start-block evidence, distinct database/storage/secrets, and updated authentication and indexer configuration; it is not complete merely because a C-Chain contract exists.
